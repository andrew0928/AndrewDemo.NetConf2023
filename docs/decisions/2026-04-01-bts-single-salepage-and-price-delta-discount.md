...
先前 BTS 技術提案曾假設一般入口與 BTS 入口會拆成不同 `SalePage`，且 checkout 需有專屬 validation service 來阻擋活動過期的結帳。但目前 `.Core.CheckoutService` 並沒有 generic business validation hook，可直接插入 `BtsCheckoutValidationService`。
...
- 由 `BtsDiscountRule` 根據 `bts-price`、gift option discount、member qualification、活動時間窗來計算價差
- 只有從 BTS 入口加入購物車的 line，才有資格被套用 BTS 折扣
- 若結帳當下活動失效或資格不符：
  - 不阻擋 checkout
  - 不套用 BTS 折扣
  - 可回傳 `Hint` 提示優惠已失效
...
- `BtsCartLineRecord` 仍然必要，用來保留「是否從 BTS 入口加入」的語意
...
1. 更新 BTS 正式 spec 與 testcase，使其對齊單一 `SalePage` 模式。
