...
| [2026-04-01](2026-04-01-bts-campaign-technical-boundary.md) | **Apple BTS Campaign 技術邊界與 Cart/Projection 重構方向**（proposed）。目前提案主張：先保留 `products` 作為 `SalePage` 主資料，透過 sidecar collections 與 `AppleBTS Extension` 補 BTS 規則；`.Core` 的首要重構點是把 `Cart` 從 `ProdQtyMap` 改為 line-based aggregate。 |
