...
| [2026-03-31](2026-03-31-bts-poc-host-side-extension-boundary.md) | **BTS POC 採 host-side 擴充，不重開 shared contract**（proposed）。這份文件整理 Apple BTS POC 的第一版邊界切法，定位為 host-side module，作為後續重新討論前的暫存設計基線。 |
| [2026-03-31](2026-03-31-shared-member-and-product-master-with-shop-projections.md) | **共享會員與商品主檔，shop 只做 projection 與 extension**（superseded）。這份文件整理過 shared master 與 price projection 的替代方向，但已不作為目前主線基準；目前主線以 tenant isolation mode 為準。 |
| [2026-03-31](2026-03-31-core-internal-projection-and-extension-strategy.md) | **`.Core` 採 internal projection pipeline 與 extension merge，`.Abstract` 保持 frozen**（superseded）。這份文件描述 shared physical database 前提下的 `.Core` 重構方向，但已不作為目前主線基準。 |
| [2026-04-01](2026-04-01-distinct-shop-ids-with-shared-physical-database.md) | **Apple Store / Apple BTS 採不同 `ShopId`，但共用同一個實體資料庫**（superseded）。這是 shared physical database 的替代方向；目前主線不採用。 |
